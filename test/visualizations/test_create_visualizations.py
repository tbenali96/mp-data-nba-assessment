import os
import pandas as pd
from src.visualizations.create_visualizations import (
...
class TestVisualizations:
    def test_plot_target_distribution_creates_an_image(self):
        ...
        # then
        assert os.path.exists("target_distribution.png") is True

    def test_plot_correlation_matrix_creates_an_image(self):
        # given
        dataframe = pd.DataFrame(
            {'col1': [1, 2, 3, 4], 'col2': [0, 0, 1, 1], 'col3': [10, 8, 20, 15]}
        )
        ...
        # then
        assert os.path.exists("correlation_matrix.png") is True

    def test_plot_histogram_points_per_game_creates_an_image(self):
        # given
        dataframe = pd.DataFrame(
            {'col1': [1, 2, 3, 4], 'target': [0, 0, 1, 1], 'PTS': [10, 8, 20, 15]}
        )
        ...
        # then
        assert os.path.exists("points_per_game.png") is True
